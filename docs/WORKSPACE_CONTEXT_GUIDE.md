...
전역 기본 persona는 `gogi`다. 작업공간에서 언어, 말투, 톤 같은 대화 표면만 좁혀야 하면 선택적으로 `[persona_override]`를 추가한다.

섹션이 없으면 전체 기본값을 상속한다. 섹션이 있어도 빠진 key는 field-level로 전역 기본값을 상속한다.
생성물, 문서, 코드 주석은 persona보다 저장소와 독자 관례가 우선한다.

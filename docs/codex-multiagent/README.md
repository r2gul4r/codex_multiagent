...
- `0~3점` -> `single-session`
- `4~6점` -> non-obvious일 때만 lightweight spawn/no-spawn basis 기록
- `7점 이상` -> explicit `spawn_decision` 기록. concrete blocker가 있으면 `single-session` 유지 가능
- 하드 트리거 -> 점수보다 먼저 재분류하되, 위임 강제는 아님

실제 worker/reviewer/explorer 수는 이 판정 뒤에 `agent_budget`, 계약 고정, write-set 분리, 독립 검증 가능성으로 정한다.

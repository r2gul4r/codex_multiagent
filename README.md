...
Every non-trivial workspace task follows `plan -> classify -> freeze -> implement -> verify -> retrospective`. `score_total` is only a complexity/risk prior. Evaluation depth and delegation value are separate decisions: delegation is considered only when hard triggers, ownership clarity, independent verification, handoff cost, authorization, and `agent_budget` justify it.
...
현재 포함된 설정은 역할을 고정 캡으로 묶지 않고, 작업 점수와 선택된 규칙에 따라 delegation 가능성을 기록하도록 맞춘다. 메인 세션 모델은 변경하지 않는다.
...
- 예산은 `score_total`, `write_set` 분리 가능성, `execution_topology`, `orchestration_value`, `hard_triggers` 로 산정한다
- 실제 spawn 여부는 authority, handoff cost, ownership clarity, discovery separability, verification independence, rework risk 를 함께 본다
...
Score band는 blind switch가 아니라 candidate gate다.

- `0~3점`: 보통 `single-session`
- `4~6점`: delegation/evaluation candidate. 선택이 애매할 때만 lightweight spawn/no-spawn 또는 evaluation basis를 남긴다
- `7점 이상`: compact evaluation plan과 explicit `spawn_decision`을 남긴다. concrete blocker가 있으면 `single-session` 유지 가능
- 하드 트리거: 점수보다 먼저 재분류하되, 위임 강제는 아님

세 축은 분리해서 본다.

- `score_total`: complexity/risk prior only
- `evaluation_need`: `none | light | full`
- `orchestration_value`: `low | medium | high`

High score alone does not upgrade `evaluation_need`, high score alone does not justify delegation, and file count alone upgrades neither axis. 실제 worker/reviewer/explorer 수는 이 candidate gate 뒤에 `orchestration_value`, `agent_budget`, 계약 고정, write-set 분리, 독립 검증 가능성, handoff cost 로 정한다. Hard checks가 있으면 LLM review보다 우선한다.
...
- `selected_rules`, `selected_skills`, `execution_topology`, `orchestration_value`, `agent_budget`, `evaluation_need` 추적
...
- default는 `single-session` profile 이고 필요할 때만 delegation 을 켠다
- 작업 크기는 `하드 트리거 먼저 + 점수제` 로 분류
- `selected_rules` 가 `review_required` 를 켤 때만 reviewer 를 필수로 만든다
- `selected_skills` 는 사용자 명령이 아니라 작업 상태에 따라 자동 선택한다
- 큰 작업의 쓰기 변경은 계약이 고정되고 write set 이 분리될 때만 `feature worker` 와 `worker_shared` 로 분리한다
- 공유 계약은 fan-out 전에 `main` 이 먼저 고정
- 멀티스텝 작업은 `STATE.md` 로 `writer_slot`, `contract_freeze`, `write_sets`, `selected_rules`, `selected_skills`, `execution_topology`, `orchestration_value`, `agent_budget`, `evaluation_need` 까지 추적
...
전역 설치는 로컬 Codex 환경의 기본 orchestration 규칙을 깔고, `ApplyWorkspace`는 각 프로젝트에 project-local `AGENTS.md`와 `STATE.md`를 만든다. 실제 루프는 설치된 각 workspace의 task 단위로 돈다.

모든 non-trivial workspace task는 `plan -> classify -> freeze -> implement -> verify -> retrospective` 흐름을 쓴다. Bounded repair와 compact retrospective도 toolkit maintenance 전용이 아니라 현재 workspace task에 붙는 task-level loop다.
...
- default는 `single-session` profile
- 작업 크기는 `하드 트리거 먼저 + 점수제` 로 분류
- 큰 작업은 조건이 맞을 때만 `delegated-serial`, `delegated-parallel`, `mixed` 중 하나로 전환
- `explorer`, `reviewer` 는 필요할 때만 활성화되는 역할로 유지
- 큰 작업의 쓰기 변경은 필요할 때 `feature worker` 와 `worker_shared` 로 분리
- `STATE.md` 로 `current_task`, `writer_slot`, `contract_freeze`, `write_sets`, `selected_rules`, `selected_skills`, `execution_topology`, `orchestration_value`, `agent_budget`, `evaluation_need` 를 추적

여기서 실행 프로필은 이렇게 본다.

- `single-session` = 기본 경로, `main`만 직접 수정하는 경로
- `delegated-serial` = 순차 위임의 이득이 handoff cost 보다 큰 경로
- `delegated-parallel` = 계약 고정, disjoint write sets, shared owner, 독립 검증, main read-only, `agent_budget > 0` 이 모두 맞는 경로
...
- `single-session = default, main 직접 수정`
- `delegated-serial = 순차 위임이 handoff cost를 이기는 경우`
- `delegated-parallel = 병렬 게이트가 모두 통과된 경우`
- `mixed = 순차 + 병렬 혼합`
...
### 8. Recursive Improvement Scope

재귀개선은 두 층으로만 본다.

- task-local recursive improvement = 현재 task의 pinned write set, tests/docs, verification surface 안에서만 하는 bounded repair 또는 closing work
- global-kit rule evolution = policy, workflow, delegation rules, installer/default templates, permission language, recording fields 같은 kit-level surface에 대한 proposal-only 흐름

Task-local loop는 새 objective를 만들지 않는다. Global-kit loop는 사용자가 kit-level 구현을 명시적으로 요청하지 않는 한 제안에 머문다.

이 킷은 background autonomous optimizer 가 아니다. polling, daemonized self-edit, cross-workspace learning, unrelated repo auto-edit, empirical self-tuning engine 을 도입하지 않는다. 효율 calibration은 structural justification과 task retrospective evidence 수준으로만 다룬다.
...
- [AGENTS.md](./AGENTS.md)
  킷 소스 체크아웃에서는 전역 기본 규칙 원본이다. 작업공간에 설치된 `AGENTS.md` 는 repo-local override 이므로 public toolkit canonical spec 로 승격하지 않는다.
- [Makefile](./Makefile)
  저장소 루트 검증 엔트리포인트
...
도구 실행 결과를 다음 자가개선 단계에 넘길 때는 `scripts/normalize_quality_signals.py` 를 기준 정규화 헬퍼로 쓴다.
...
```bash
python3 scripts/normalize_quality_signals.py --input examples/quality_signal_samples.json --history /tmp/quality-history.json --store --pretty
python3 scripts/normalize_quality_signals.py --history /tmp/quality-history.json --query latest --pretty
python3 scripts/normalize_quality_signals.py --history /tmp/quality-history.json --query summary --pretty
```
...
- `make test`
  ...
  - `python3 scripts/normalize_quality_signals.py --input examples/quality_signal_samples.json` 샘플 기반 정규화 검사
  - 샘플 입력을 임시 `--history` 파일에 저장한 뒤 `latest`, `summary` 조회까지 스모크 검사
  - `python3 scripts/extract_test_gap_candidates.py --root .` 로 테스트 공백 리포트가 재현되는지 검사
- `make check`
  - `make lint` + `make test`
- `make test-test-gap-docs`
  - `scripts/extract_test_gap_candidates.py` 의 JSON/Markdown 출력이 현재 저장소 상태와 일치하는지 검사
...
- 사용자나 workspace instruction 이 허용한 범위 안에서 delegation 판단을 기록하고 싶을 때

...
이 저장소 문서가 권하는 워크스페이스 오버라이드 패턴은 `STATE.md` 를 루트 보드로 두고, `state/TASK-*.md` 같은 task-state 파일로 스레드별 상세를 분리하는 방식이다.
`WORKSPACE_CONTEXT.toml` 에서는 `task_state_dir = "state/"` 로 이 경로를 드러내고, 필요하면 다른 디렉터리로 override 하면 된다.
루트 보드의 기본 어휘는 `active_tasks`, `blocked_tasks`, `owned_write_sets`, `task_state_dir` 쪽으로 맞추는 게 덜 헷갈린다.
이건 작업공간 오버라이드용 패턴이지, 전역 기본 설치가 반드시 같은 구조를 강제한다는 뜻은 아니다.
...
- root `STATE.md` 보드 + `state/TASK-*.md` task-state 파일
- `route + writer_slot + owned_write_sets` 기반 실행 소유권 흐름
...
- 멀티스텝 또는 동시 작업은 root `STATE.md` 보드와 `state/TASK-*.md` task-state 파일로 분리해서 추적
- 큰 작업이 끝날 때 reviewer 확인 절차를 거치도록 유도
...
실제 동시 작업을 굴릴 때는 이 루트 보드에 더해 `state/TASK-*.md` 같은 task-state 파일을 별도로 두는 쪽이 충돌을 덜 낸다.
새 workspace override 가 starter task-state 템플릿을 `state/` 아래에 함께 생성하는 경우도 있는데, 그 경우에도 루트 보드는 ownership registry 역할만 남기는 게 맞다.
이 문서가 설명하는 root-board + task-state 모델은 작업공간 오버라이드에서 쓰는 운영 패턴이지, 전역 기본값 자체를 다시 정의하는 건 아니다.
...
- 작은 작업은 `main` 단독 진행
- 작업 크기는 `하드 트리거 + 점수제` 로 먼저 분류
- 큰 작업은 `Route B` delegated mode 로 전환
- `explorer`, `reviewer` 는 read-only 역할로 유지
- 큰 작업의 쓰기 변경은 필요할 때 `feature worker` 와 `worker_shared` 로 분리
- root `STATE.md` 는 등록, blocked/unblocked, ownership claim/release, closeout 같은 보드 이벤트만 기록
- 스레드별 실행 상세는 `state/TASK-*.md` 에 기록

여기서 라우트는 이렇게 본다.

- `Route A` = 작은 작업, `main`만 직접 수정하는 main-only 경로
- `Route B` = 큰 작업, `main` 은 수정 안 하고 계약 고정/분배만 수행
...
무거운 큐 시스템 대신 가벼운 root `STATE.md` 보드와 task-state 파일로 관리한다.

- `active_tasks`
- `blocked_tasks`
- `owned_write_sets`
- `task_state_dir = state/`
- root 보드는 등록, blocked/unblocked, ownership claim/release, closeout 만 담당
- `state/TASK-*.md` 는 task id, owner thread, scope, write set, reviewer, verification notes 를 담는다
...
- `Route B = 큰 작업, main planner-only + worker 분배`

### 3. Writer Slot + Write Sets

`Route A` 에선 단일 수정 주체를 `writer_slot` 으로 기록한다.

- `writer_slot = free`
- `writer_slot = main`
- `writer_slot = worker_name`

`Route B` 에선 보드 쪽은 `owned_write_sets` 로 두고, Writer Slot 섹션에서는 `writer_scope = parallel` 과 `worker_ownership_map` 으로 명시한다.

- `worker_feature_ui = [owned file globs]`
- `worker_feature_api = [owned file globs]`
- `worker_shared = [shared asset paths only]`
- 각 worker 는 보통 자기 task-state 파일만 상세 갱신한다

### 4. Contract Freeze

공유 계약은 `Route B` fan-out 전에, 또는 `Route A` handoff 전에 `main` 이 먼저 고정한다.
...
workspace override 쪽에서 concurrent threads 를 쓸 때는 `STATE.md` 를 루트 보드로 두고 `state/TASK-*.md` 로 스레드별 상세를 나누는 패턴을 권장한다.

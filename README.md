...
- 기본은 `main` 단독 진행
- 멀티에이전트는 분리가 실제로 더 안전할 때만 사용
- `explorer`, `reviewer` 는 read-only 유지
- 코드나 파일을 쓰는 역할은 항상 하나만 유지
- 공유 계약은 fan-out 전에 `main` 이 먼저 고정
- 멀티스텝 작업은 `STATE.md` 로 추적
...
- 기본은 `main` 단독 진행
- `write scope` 가 겹치면 병렬화하지 않음
- `explorer`, `reviewer` 는 read-only 역할로 유지
- 쓰기 변경은 reviewer 확인 절차로 닫음
- 동시 상한은 `explorer 3`, `reviewer 2`, `writer 1`
- `STATE.md` 로 `current_task`, `writer_slot`, `contract_freeze` 를 추적

여기서 `writer` 는 실제로 코드나 파일을 수정하는 역할 전체를 뜻한다.

- `main` 이 직접 수정하면 `main` 이 writer 슬롯을 사용
- `worker` 에 위임하면 해당 `worker` 가 writer 슬롯을 사용
- 둘이 동시에 쓰는 것은 허용하지 않음

---

## 빠른 시작

Windows PowerShell 을 관리자 권한으로 열고 아래 둘 중 하나를 실행하면 된다.
...
macOS 에서는 `bash` 기준으로 한 줄 설치가 가능하다.

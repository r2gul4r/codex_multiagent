...
- `4~6점`: delegation candidate. 선택이 애매할 때만 lightweight spawn/no-spawn basis를 남긴다
- `7점 이상`: explicit `spawn_decision`을 남긴다. concrete blocker가 있으면 `single-session` 유지 가능
- 하드 트리거: 점수보다 먼저 재분류하되, 위임 강제는 아님

실제 worker/reviewer/explorer 수는 이 candidate gate 뒤에 `agent_budget`, 계약 고정, write-set 분리, 독립 검증 가능성, handoff cost 로 정한다.

...
- `v0.1.1 - 2026-03-18`
- 전역/작업공간 기본 페르소나 `gogi` 적용
- Codex 서브에이전트 `default`, `worker`, `explorer`, `reviewer` 모델을 `gpt-5.4-mini` 로 패치
- Codex 기본 rules 파일로 파괴적 삭제/되돌리기 명령 기본 차단
...
Antigravity 쪽은 Codex처럼 새 모델을 설치하는 방식 아님
...
즉 설치 결과는 멀티에이전트 역할과 분할 규칙을 런타임에 주입하는 쪽임
